...
char ddmmBuffer[] = "00-00", yyyyBuffer[] = "0000", hhmmBuffer[] = "00:00",
     ssBuffer[] = "00", wdBuffer[] = "XXXX";
...
static void timerCallbackBattery(void *data) {
  if (CfgData.s_Charging) {
    ...
    batteryAkt =
        gbitmap_create_as_sub_bitmap(batteryAll, GRect(0, 10 * nImage, 20, 10));
...
void battery_state_service_handler(BatteryChargeState charge_state) {
  ...
  batteryAkt =
      gbitmap_create_as_sub_bitmap(batteryAll, GRect(0, 10 * nImage, 20, 10));
  bitmap_layer_set_bitmap(battery_layer, batteryAkt);
  layer_set_hidden(bitmap_layer_get_layer(battery_layer),
                   !CfgData.s_Charging &&
                       (CfgData.battdgt || aktBatt > CfgData.showbatt));
  update_all();
}
//-----------------------------------------------------------------------------------------------------------------------
...
static void background_layer_update_callback(Layer *layer, GContext *ctx) {
  ...
  graphics_draw_bitmap_in_rect(ctx, background,
                               GRect(0, 0, bg_size.w, bg_size.h));

  // DD
  GBitmap *bmpTmp =
      gbitmap_create_as_sub_bitmap(batteryAll, GRect(0, 115, 12, 5));
  graphics_draw_bitmap_in_rect(ctx, bmpTmp,
                               GRect(CfgData.datefmt != 3 ? 17 : 48, 5, 12, 5));
  gbitmap_destroy(bmpTmp);

  // MM
  bmpTmp = gbitmap_create_as_sub_bitmap(batteryAll, GRect(0, 120, 12, 5));
  graphics_draw_bitmap_in_rect(ctx, bmpTmp,
                               GRect(CfgData.datefmt != 3 ? 48 : 17, 5, 12, 5));
  gbitmap_destroy(bmpTmp);
  ...
  if (CfgData.weather) {
    if (CfgData.w_time != 0) {
      ...
      graphics_draw_text(ctx, !CfgData.isunit ? "_" : "`", WeatherF,
                         GRect(72, 34 - (CfgData.cond ? 2 : 0), 18, 32),
                         GTextOverflowModeFill, GTextAlignmentCenter, NULL);

      GSize sz = graphics_text_layout_get_content_size(
          CfgData.w_icon, WeatherF, rc, GTextOverflowModeFill,
          GTextAlignmentCenter);
      graphics_draw_text(ctx, CfgData.w_icon, WeatherF,
                         GRect(rc.origin.x + rc.size.w / 2 - sz.w / 2,
                               rc.origin.y + rc.size.h / 2 - sz.h / 2, sz.w,
                               sz.h),
                         GTextOverflowModeFill, GTextAlignmentCenter, NULL);

      if (CfgData.cond) {
        // sz = graphics_text_layout_get_content_size(CfgData.w_cond, arial9,
        // GRect(5, 65, 108, 10), GTextOverflowModeTrailingEllipsis,
        // GTextAlignmentCenter);
        graphics_draw_text(ctx, CfgData.w_cond, arial9, GRect(5, 66, 108, 10),
                           GTextOverflowModeTrailingEllipsis,
                           GTextAlignmentCenter, NULL);
      }
    } else {
      GSize sz = graphics_text_layout_get_content_size(
          "h", WeatherF, rc, GTextOverflowModeFill, GTextAlignmentCenter);
      graphics_draw_text(ctx, "h", WeatherF,
                         GRect(rc.origin.x + rc.size.w / 2 - sz.w / 2,
                               rc.origin.y + rc.size.h / 2 - sz.h / 2, sz.w,
                               sz.h),
                         GTextOverflowModeFill, GTextAlignmentCenter, NULL);
  ...
  if (!clock_is_24h_style()) {
    bmpTmp = gbitmap_create_as_sub_bitmap(
        batteryAll, GRect(CfgData.isAM ? 0 : 4, 125, 4, 5));
  ...
  if (!CfgData.s_Charging && CfgData.battdgt && aktBatt <= CfgData.showbatt) {
    ...
    graphics_draw_text(ctx, sTemp, arial9, GRect(115, 90, 25, 10),
                       GTextOverflowModeFill, GTextAlignmentCenter, NULL);
  }
}
//-----------------------------------------------------------------------------------------------------------------------
void tick_handler(struct tm *tick_time, TimeUnits units_changed) {
  ...
  if (seconds == 0 || units_changed == MINUTE_UNIT) {
    ...
    if ((tick_time->tm_hour == 4 && tick_time->tm_min == 0) ||
    ...
    if (CfgData.inv)
      layer_add_child(sec_window_layer,
                      inverter_layer_get_layer(sec_inv_layer));
...
static void timerCallbackWeather(void *data) {
  if (CfgData.w_UpdateRetry &&
      !layer_get_hidden(bitmap_layer_get_layer(radio_layer))) {
    update_weather();
    timer_weather = app_timer_register(30000, timerCallbackWeather,
                                       NULL); // Try again in 30 sec
  } else {
    CfgData.w_UpdateRetry = true;
    timer_weather = app_timer_register(60000 * 60, timerCallbackWeather,
                                       NULL); // 1h static update
  }
}
//-----------------------------------------------------------------------------------------------------------------------
static void update_configuration(void) {
  ...
  app_log(APP_LOG_LEVEL_DEBUG, __FILE__, __LINE__,
          "Curr Conf #1: inv:%d, showsec:%d, battdgt:%d, showbatt:%d, vibr:%d, "
          "datefmt:%d, weather:%d",
          CfgData.inv, CfgData.showsec, CfgData.battdgt, CfgData.showbatt,
          CfgData.vibr, CfgData.datefmt, CfgData.weather);
  app_log(APP_LOG_LEVEL_DEBUG, __FILE__, __LINE__,
          "Curr Conf #2: isunit:%d, cityid:%d", CfgData.isunit,
          (int)CfgData.cityid);
  app_log(APP_LOG_LEVEL_DEBUG, __FILE__, __LINE__,
          "Weather Data: w_time:%d, w_temp:%d, w_icon:%s, w_cond:%s",
          (int)CfgData.w_time, CfgData.w_temp, CfgData.w_icon, CfgData.w_cond);
  ...
  if (CfgData.w_UpdateRetry && CfgData.weather) {
    if (CfgData.w_time == 0 || (tmAkt - CfgData.w_time) > 60 * 60)
      ...
      timer_weather =
          app_timer_register((60 * 60 - (tmAkt - CfgData.w_time)) * 1000,
                             timerCallbackWeather, NULL);
  }
}
//-----------------------------------------------------------------------------------------------------------------------
void in_received_handler(DictionaryIterator *received, void *ctx) {
  ...
  while (akt_tuple) {
    ...
    app_log(APP_LOG_LEVEL_DEBUG, __FILE__, __LINE__, "KEY %d=%d/%s/%d",
            (int16_t)akt_tuple->key, akt_tuple->value->int16,
            akt_tuple->value->cstring, intVal);
    ...
    case W_TEMP:
      ...
      CfgData.w_UpdateRetry = false; // Update successful, usual update wait
                                     // time
      break;
    case W_ICON:
...
void in_dropped_handler(AppMessageResult reason, void *ctx) {
  app_log(APP_LOG_LEVEL_WARNING, __FILE__, __LINE__,
          "Message dropped, reason code %d", reason);
}
//-----------------------------------------------------------------------------------------------------------------------
void window_load(Window *window) {
  ...
  digitS =
      fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_DIGITAL_25));
  digitM =
      fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_DIGITAL_35));
  digitL =
      fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_DIGITAL_55));
  WeatherF =
      fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_WEATHER_32));
  arial9 = fonts_load_custom_font(
      resource_get_handle(RESOURCE_ID_FONT_ARIAL_BOLD_9));
